...
function createBotInstance({ slug, config, onTick, onTrade, onLog, onMarketLoaded }) {
  ...
  function trySell(assetId, bid, ask) {
    ...
    if (!pos) return
    // Use bid for sell evaluation — it's the actual exit price
    ...
    const entry = { type: 'SELL', outcome: pos.outcomeName, shares: pos.shares, buyAsk: (pos.buyAsk*100).toFixed(1), sellAsk: (exitPrice*100).toFixed(1), peakAsk: (pos.peakAsk*100).toFixed(1), pnl: pnl.toFixed(2), reason: sellReason, balanceAfter: paperBalance.toFixed(2), time: new Date().toLocaleTimeString(), assetId }
    tradeLog.push(entry)
  ...
  function processTick(assetId, outcomeName, rawMid, rawBid, rawAsk, source) {
    ...
    // ✅ FIX: check sell FIRST on every tick (catches stop-loss before a new buy sneaks in)
  ...
  function connectWebSocket() {
    ...
    // ✅ Heartbeat: required or server drops connection after ~10s
    ...
    ws.onopen = () => {
      ...
      pingInterval = setInterval(() => { if (ws.readyState === 1) ws.send('PING') }, 9000)
    }
    ...
    ws.onmessage = (event) => {
      try {
        ...
        // ✅ FIX 1: Handle best_bid_ask — fastest signal, fires on every orderbook touch
        ...
        // ✅ FIX 2: Handle price_change — use best_bid and best_ask fields, not just price
        if (msg.event_type === 'price_change') {
          ...
          for (const change of priceChanges) {
            ...
            processTick(assetId, outcomeName, rawMid, rawBid, rawAsk, 'WS-PC')
          }
          return
        }

        // book snapshot — update our cached prices
  ...
  async function pollPrice(tokenId, outcomeName) {
    try {
      // ✅ FIX 3: Use /book endpoint for bid+ask instead of /price (mid only)
      ...
      processTick(tokenId, outcomeName, 0, topBid, topAsk, 'POLL')
    } catch (e) {}
  }

  async function startPolling() {
    // ✅ FIX 4: Poll every 2s instead of 5s to cut fallback latency
